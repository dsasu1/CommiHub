...
export class PropertyService extends AbstractRestService {
  ...
  propInfoListChange: BehaviorSubject<
    PropertyInformation[]
  > = new BehaviorSubject<PropertyInformation[]>(null);
  propAccessListChange: BehaviorSubject<PropertyAccess[]> = new BehaviorSubject<
    PropertyAccess[]
  >(null);
  ...
  savePropertyInfo(propInfo: PropertyInformation) {
    return this.postItem<PropertyInformation>(
      this.controller + "SavePropertyInfo",
      propInfo
    );
  }
  ...
  getPropertyHours(propertyId: string) {
    ...
    return this.getItem<HourVM>(
      this.controller + "GetPropertyHours",
      httpParam
    );
  }
  ...
  getPropertyByUrl(urlName: string) {
    ...
    return this.getItem<PropertyInformation>(
      this.controller + "GetPropertyByUrl",
      httpParam
    );
  }
  ...
  getUserProperties(userId: string) {
    ...
    return this.getItem<PropertyInformation[]>(
      this.controller + "GetUserProperties",
      httpParam
    );
  }
  ...
  deleteProperty(userId: string, propertyId: string) {
    ...
    return this.deleteItem<boolean>(
  ...
  deletePropertyAccess(userId: string, propertyId: string, roleId: string) {
    ...
    return this.deleteItem<boolean>(
      this.controller + "DeletePropertyAccess",
      httpParam
    );
  }
  ...
  getPropertyAccess(userId: string, roleId: string) {
    let httpParam: HttpParams = new HttpParams()
      .append("userId", userId)
      .append("roleId", roleId);
    return this.getItem<PropertyAccess[]>(
      this.controller + "GetPropertyAccess",
      httpParam
    );
  }
